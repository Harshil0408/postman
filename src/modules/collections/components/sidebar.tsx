import { useGetWorkspaceById } from "@/modules/workspaces/hooks/workspaces";
...
interface Props {
  currentWorkspace: Awaited<ReturnType<typeof useGetWorkspaceById>>["data"];
}
...
const TabbedSidebar = ({ currentWorkspace }: Props) => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "Collections":
        return (
          <div className="h-full bg-zinc-950 text-zinc-950 flex flex-col">
            ...
            <div className="p-4 border-b border-zinc-800">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search"
                  className="w-full bg-zinc-900 border-zinc-700 rounded-lg pl-10 pr-2 py-3 text-sm text-zinc-100 placeholder:zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                />
              </div>
            </div>
            <div className="p-4 border-b border-zinc-800">
    ...
    <div className="flex h-screen bg-zinc-900">
      <div className="w-12 bg-zinc-900 border-r border-zinc-800 flex flex-col items-center py-4 space-y-4">
        {sidebarItems.map((item, index) => (
          <Hint label={item.label} key={index}>
            <div
